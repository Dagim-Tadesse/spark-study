...
const MarkdownRenderer = ({ content, className }: { content: string; className?: string }) => {
  ...
  if (content.includes("<") && content.includes(">")) {
    return (
      <div 
        className={cn("prose prose-sm dark:prose-invert max-w-none w-full break-words", className)}
  ...
  const renderLine = (line: string, i: number) => {
    ...
    if (imgMatch) return <img key={i} src={imgMatch[2]} alt={imgMatch[1]} className="max-w-full max-h-[300px] rounded-xl shadow-lg my-4 object-contain mx-auto" />;
    
    if (line.startsWith("# ")) return <h1 key={i} className="text-2xl font-black border-b border-border/50 pb-2 mb-4 mt-6">{line.slice(2)}</h1>;
  ...
  return <div className="space-y-3 w-full py-2">{content.split('\n').map((line, i) => renderLine(line, i))}</div>;
};
...
const Decks = () => {
  ...
  const handleSelectCard = (card: Card) => {
    ...
    setTimeout(() => {
      if (editorRef.current) editorRef.current.innerHTML = activeSide === "front" ? card.front : card.back;
    }, 0);
    setShowSafety(false);
    setShowMoveDialog(false);
  };
  ...
  const handleEditorAction = (action: string) => {
    ...
    switch (action) {
      ...
      case "math": 
        ...
        if (math) document.execCommand("insertHTML", false, `<span class="math-tex text-orange-500 font-bold">$$ ${math} $$</span>`);
        break;
      case "image-upload": triggerFileAction("image"); return;
      case "tts": 
  ...
  const addDeck = async () => {
    ...
    try {
      const newDeck = await deckService.createDeck(user.id, newDeckName, "bg-primary");
          ...
          {isCreatingDeck && (
            <div className="bg-primary/5 p-4 rounded-2xl border border-primary/20 space-y-3 animate-in slide-in-from-top-2">
              ...
              <div className="flex gap-2">
                <button onClick={addDeck} className="flex-1 bg-primary text-white py-2 rounded-xl text-xs font-black">Create</button>
                <button onClick={() => setIsCreatingDeck(false)} className="flex-1 bg-secondary py-2 rounded-xl text-xs font-black">Cancel</button>
              </div>
            </div>
          )}
          ...
          <div className="space-y-2">
              ...
              <div key={deck.id} className="relative group">
                ...
                >
                  ...
                  <p className={cn(
                    ...
                    selectedDeckId === deck.id ? "text-white/70" : "text-muted-foreground"
                  )}>
                    {cards.filter(c => c.deck_id === deck.id).length} {t("library.cards")}
                  </p>
                </button>
                ...
                <div className={cn("absolute right-2 top-1/2 -translate-y-1/2 transition-opacity", selectedDeckId === deck.id ? "opacity-100" : "opacity-0 group-hover:opacity-100")}>
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <button onClick={(e) => e.stopPropagation()} className={cn("p-1.5 rounded-lg transition-colors", selectedDeckId === deck.id ? "hover:bg-white/20 text-white" : "hover:bg-secondary text-muted-foreground hover:text-foreground")}>
                        <MoreVertical className="size-4" />
                      </button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent align="end" className="w-48 rounded-2xl p-2 shadow-xl border-border">
                      <DropdownMenuItem onClick={(e) => startEditingDeck(deck, e as any)} className="rounded-xl font-bold p-3">
                        <Edit2 className="size-4 mr-3" /> Rename Deck
                      </DropdownMenuItem>
                      <DropdownMenuItem className="text-destructive rounded-xl font-bold p-3" onClick={(e) => confirmDeleteDeck(deck.id, e as any)}>
            ...
            {filteredDecks.length === 0 && (
              <div className="py-12 text-center bg-secondary/5 rounded-3xl border-2 border-dashed border-border/30">
                ...
                <p className="text-sm font-bold text-muted-foreground">{t("library.noDecks")}</p>
              </div>
            )}
          </div>
        </aside>
          ...
          {selectedDeckId ? (
            <>
              <div className="flex items-center justify-between bg-card p-4 rounded-3xl border border-border shadow-sm">
                <div className="flex items-center gap-4">
                  <button onClick={() => setSelectedDeckId(null)} className="lg:hidden p-2 rounded-xl hover:bg-secondary transition-colors">
                    <ArrowLeft className="size-5" />
                  </button>
                  <div>
                    <h3 className="text-lg font-black truncate max-w-[200px] sm:max-w-none">{decks.find(d => d.id === selectedDeckId)?.name}</h3>
                    <div className="flex items-center gap-2 mt-1">
                      <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{cards.filter(c => c.deck_id === selectedDeckId).length} {t("library.cards")}</p>
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <input 
                          ...
                          value={decks.find(d => d.id === selectedDeckId)?.tags?.join(", ") || ""}
                          onChange={(e) => {
                            const tags = e.target.value.split(",").map(t => t.trim());
                            setDecks(prev => prev.map(d => d.id === selectedDeckId ? { ...d, tags } : d));
                ...
                <div className="flex-1 flex flex-col gap-4 bg-card border border-border rounded-[2.5rem] p-6 shadow-sm min-h-0 min-w-0">
                  ...
                  <div className="flex-1 relative min-h-[300px] overflow-hidden">
                    <div
                      ref={editorRef}
                      contentEditable
                      onInput={(e) => handleUpdateContent({ [activeSide]: e.currentTarget.innerHTML })}
                        ...
                        {["Definition", "Formula", "Q&A", "Diagram"].map(tag => (
                          <button
                            ...
                            className={cn(
                              ...
                              localTag === tag ? "bg-primary text-white border-primary" : "bg-secondary/50 text-muted-foreground border-border hover:border-primary/50"
                            )}
                          >
                            {tag}
                          </button>
                    ...
                    <div className="flex-1 overflow-y-auto pr-1 space-y-3 custom-scrollbar min-h-0">
                      {deckCards.map(c => (
                        <button
                          ...
                          className={cn(
                            ...
                            selectedCardId === c.id ? "border-primary bg-primary/5 ring-2 ring-primary/20 shadow-md scale-[1.01]" : "border-border bg-card/50 hover:border-primary/40 hover:bg-card hover:shadow-lg"
                          )}
                        >
                          <div className="space-y-2">
                            <p className="text-[10px] font-black text-primary uppercase tracking-widest opacity-60">{t("editor.front")}</p>
                             <div className="flex flex-col gap-1.5 flex-1 min-w-0">
                               <p className={cn("font-bold text-sm truncate", selectedCardId === c.id ? "text-primary" : "text-foreground")}>
                                 {c.front.replace(/<[^>]*>/g, '').slice(0, 40) || "Empty Card"}
                               </p>
                               {c.tags && c.tags.length > 0 && (
                                 <div className="flex flex-wrap gap-1">
                                   {c.tags.map(t => (
                                     <span key={t} className="px-1.5 py-0.5 rounded-md bg-primary/10 text-primary text-[8px] font-black uppercase">
                                       {t}
                                     </span>
                                   ))}
                                 </div>
                          ...
                          <div className="flex items-center justify-between mt-auto pt-4 border-t border-border/50">
                             <div className="flex gap-2 items-center">
                               {c.front.includes("![image]") || c.back.includes("![image]") ? <div className="p-1.5 rounded-lg bg-primary/5 text-primary"><ImagePlus className="size-3.5" /></div> : null}
                               {c.front.includes("[audio]") || c.back.includes("[audio]") ? <div className="p-1.5 rounded-lg bg-emerald-500/5 text-emerald-500"><Volume2 className="size-3.5" /></div> : null}
                               {c.front.includes("$$") ? <div className="p-1.5 rounded-lg bg-orange-500/5 text-orange-500"><Sigma className="size-3.5" /></div> : null}
                             </div>
                             <div className="flex items-center gap-1">
                               <button 
                               ...
                               <ChevronRight className={cn("size-5 text-muted-foreground group-hover:text-primary transition-transform duration-300", selectedCardId === c.id ? "translate-x-0" : "-translate-x-2 opacity-0 group-hover:opacity-100 group-hover:translate-x-0")} />
                             </div>
                          </div>
                        </button>
                      ))}
                      {!deckCards.length && (
                        <div className="py-20 flex flex-col items-center justify-center text-center bg-secondary/5 rounded-3xl border-2 border-dashed border-border/30">
                          ...
                          <p className="text-sm font-bold text-muted-foreground">{t("editor.empty")}</p>
                        </div>
                      )}
                </div>
              </div>
            ...
            <div className="flex-1 grid place-items-center bg-card/30 border border-dashed border-border rounded-[3rem] p-12 text-center">
              <div className="max-w-xs">
                ...
                <h3 className="text-2xl font-black mb-2">{t("library.noDecks")}</h3>
                <p className="text-muted-foreground font-medium mb-8">Select a deck from the sidebar or create a new one to start building your knowledge base.</p>
                <button onClick={addDeck} className="bg-primary text-primary-foreground px-8 py-3 rounded-2xl font-black shadow-xl shadow-primary/20 hover:scale-105 transition-transform">
                  {t("common.newDeck")}
                </button>
              </div>
            </div>
