...
export const validateDeckName = (name: string): ValidationResult => {
  ...
  if (!trimmed) return { isValid: false, error: "Deck name cannot be empty" };
  if (trimmed.length > DECK_NAME_MAX_LENGTH) {
    return { isValid: false, error: `Deck name must be ${DECK_NAME_MAX_LENGTH} characters or less` };
  }
  return { isValid: true };
};
...
export const validateCardText = (text: string): ValidationResult => {
  if (text.length > CARD_TEXT_MAX_LENGTH) {
    return { isValid: false, error: `Card content must be ${CARD_TEXT_MAX_LENGTH} characters or less` };
  }
  return { isValid: true };
};
